    ...
    <div className="flex flex-col min-h-screen bg-slate-950 text-white overflow-hidden">
      ...
      <header className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
        <div className="flex items-center gap-6">
          <div className="bg-slate-800 px-4 py-2 rounded-lg font-bold text-lg tracking-widest border border-slate-700">
            {unwrappedParams.roomCode}
          </div>
          <div className="flex items-center gap-2 text-slate-300">
            <Users size={20} className="text-blue-400" />
            <span className="font-bold">{answersCount} / {totalPlayers}</span> Answers
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col p-6 max-w-6xl mx-auto w-full">
        ...
        <div className="flex-1 flex flex-col items-center justify-center mb-12 relative">
          ...
          {!isRevealed && (
            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-24 h-24 rounded-full border-4 border-slate-700 flex items-center justify-center text-3xl font-black bg-slate-900 shadow-[0_0_30px_rgba(59,130,246,0.2)]">
              {timeLeft}
            </div>
          )}
        </div>
      ...
      <div className="fixed right-4 bottom-4 w-64 max-h-96 bg-slate-900/90 border border-slate-800 rounded-2xl p-4 overflow-y-auto z-50 backdrop-blur-md shadow-2xl">
        ...
        <div className="space-y-3">
          ...
          {logs.length === 0 && <div className="text-[10px] text-slate-600 text-center py-4">No incidents detected</div>}
        </div>
      </div>
    </div>
  );
